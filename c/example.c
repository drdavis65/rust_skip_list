...
void print_level(skip_list_t *skip_list, int level) {
    // Check if level is valid
    if (level < 0 || level >= (int)skip_list->level) {
        printf("Error: Invalid level %d. Valid levels are 0 to %d\n", 
               level, skip_list->level - 1);
        return;
    }
    
    printf("Level %d keys: ", level);
    
    // Start from head and traverse the specified level
    skip_node_t *node = skip_list->head->forward[level].node;
    
    // Print all keys at this level (treating them as strings)
    while (node != NULL) {
        printf("%s ", (char*)node->key);
        node = node->forward[level].node;
    }
...
int main() {
       ...
       * key, else it will return NULL*/
       ...
       printf("Is `n` in the skip list ? %i\n",
              jrsl_search(&skip_list, "n") != NULL);

       // Test the print_level function
       printf("\n\nTesting print_level function:\n");
       print_level(&skip_list, 0);  // Print all keys at level 0
       print_level(&skip_list, 1);  // Print keys at level 1  
       print_level(&skip_list, 2);  // Print keys at level 2
       print_level(&skip_list, 3);  // This should show an error for invalid level
